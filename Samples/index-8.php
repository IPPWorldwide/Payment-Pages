...
<html>
	...
	<body>
		...
		<div id="paymentPopup" class="payment-getway-main design-8">
		...
		<script type="text/javascript">
		    jQuery(document).ready(function($) {
		    	$("body").on("focus", "input, textarea", function() {
			    	$('.payw_keyupconfirm').on('blur', function(){
						if($(this).val().length > 0) {
						   $(this).parent().siblings('.field-description').addClass('isFocus');		
						}
						else{
							$(this).parent().siblings('.field-description').removeClass('isFocus');
						}
					}).on('focus', function(){
					  	$(this).parent().siblings('.field-description').addClass('isFocus');
					});
